...
const GameForm: React.FC<GameFormProps> = ({ onGameGenerated }) => {
  ...
  useEffect(() => {
    // Adjust number of questions to match squares
          ...
          {questions.map((q, index) => (
            <div key={index} className="p-4 bg-blue-50 rounded-lg relative transform transition-all duration-200 hover:scale-[1.01] hover:shadow-md">
              <button
                ...
                className="absolute right-2 top-2 text-rose-500 hover:text-rose-700 transition-colors"
                aria-label="Remove question"
                disabled={questions.length <= squares}
              >
                <MinusCircle size={18} className={questions.length <= squares ? 'opacity-50' : ''} />
              </button>
